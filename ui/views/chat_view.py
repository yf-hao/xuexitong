...
class ChatView(QWidget):
    ...
    def _on_messages_loaded(self, sessions: list):
        ...
        selected_chat_id = self._current_target_id
    ...
    def set_students(self, students: list[dict]):
        ...
        for stu in students:
            ...
            item = QListWidgetItem(display)
            item.setData(Qt.ItemDataRole.UserRole, stu.get("person_id", ""))
            item.setData(Qt.ItemDataRole.UserRole + 1, name)
    ...
    def _on_chat_selected(self, current: QListWidgetItem, previous: QListWidgetItem):
        ...
        self._open_chat(target_id, name)
    ...
    def _open_chat(self, target_id: str, target_name: str):
        """打开与目标的聊天"""
        ...
        if hasattr(self.crawler, "is_msync_connected") and not self.crawler.is_msync_connected():
            try:
                self.crawler.connect_msync(
                    on_message=self._on_msync_message,
    ...
    def _load_current_chat_history(self, limit: int = 50):
        ...
        self._history_worker = ChatHistoryWorker(self.crawler, self._current_target_id, limit=limit)
    ...
    def _on_history_loaded(self, chat_id: str, messages: list):
        """历史消息加载完成回调。"""
        if chat_id != self._current_target_id:
            return
        ...
        for raw in messages:
            ...
            try:
                ...
                timestamp = 0
            normalized.append((timestamp, raw))
        ...
        for _, raw in normalized:
            ...
            display_name = "我" if is_self else sender_name
            self.append_message(display_name, str(content), is_self=is_self)
    ...
    def _on_msync_message(self, msg: dict):
        ...
        if self._current_target_id == from_user:
            self.append_message(from_user, content, is_self=False)
        else:
            # 否则刷新会话列表（显示未读）
            self._load_message_list()
    ...
    def _on_send(self):
        ...
        try:
            self._ensure_msync_connected()
            result = self.crawler.send_message(self._current_target_id, text, target_name=self._current_target_name or "")
